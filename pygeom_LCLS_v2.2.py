...
def readgeomfile(infilename):
    ...
    infile.close()
    return geometry,panels,adu,clen,offset

##########################WRITEGEOMFILE####################################
def writegeomfile(outfilename,geom,panels,adu,clen,offset):
    ...
    for n in range(numpanels):
        ...
        outfile.write(identifier+'res = 9097.525473 \n')
...
geom,panels,adu,clen,offset=readgeomfile(geomfilename)
...
for line in infile:
    ...
    if ('fs/px   ss/px  (1/d)/nm^-1   Intensity' in line) and (indexed==1):
        inpeaklist=1
...
writegeomfile('output.geom',newgeom,panels,adu,clen,offset)
